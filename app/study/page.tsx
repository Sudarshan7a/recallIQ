...
function EditablePreviewCard({
  ...
  const [draft, setDraft] = useState({ front: card.front, back: card.back, importance: card.importance });
  ...
  const handleSave = () => {
    ...
    onUpdate(index, { ...card, front: draft.front.trim(), back: draft.back.trim(), importance: draft.importance });
    setIsEditing(false);
    setFlipped(false);
  };
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between mb-1">
          <span className="font-label font-bold text-[10px] text-text-secondary uppercase tracking-widest">Card {index + 1}</span>
          <button onClick={() => setIsEditing(false)} className="text-text-secondary hover:text-text-primary p-1 rounded-full hover:bg-background transition-colors cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Front</label>
          <textarea
            value={draft.front}
        ...
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Back</label>
          <textarea
            value={draft.back}
        ...
        <div>
          <label className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Importance</label>
          <select
    ...
    >
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1" onClick={e => e.stopPropagation()}>
              <button onClick={() => setIsEditing(true)} className="p-1 rounded-full text-text-secondary hover:text-primary hover:bg-background transition-colors cursor-pointer">
                <Edit3 className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => onDelete(index)} className="p-1 rounded-full text-text-secondary hover:text-error hover:bg-error-light transition-colors cursor-pointer">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
              <span className={`rounded-full border px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider ${badge}`}>{label}</span>
            </div>
          </div>
          <h3 className="font-body text-sm font-semibold text-text-primary line-clamp-2 my-auto">{card.front}</h3>
        ...
        >
          ...
          <p className="font-body text-xs text-text-secondary leading-relaxed line-clamp-3 my-auto">{card.back}</p>
          <span className="text-[8px] font-bold text-text-secondary uppercase tracking-widest text-center">Tap to view question</span>
        </div>
      </motion.div>
    </motion.div>
  );
...
function StudyPageInner() {
  ...
  const handleGenerate = async () => {
    if (!canGenerate) { setError("Paste at least a few sentences."); setStatus("error"); return; }
    if (!selectedDeckId) { setError("Please select a target deck first."); setStatus("error"); return; }
    setError(""); setStatus("generating");
    try {
      const res = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: content, deck_id: selectedDeckId, domain: "general" }),
      });
      const data = await res.json();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus("saved");
      setDecks(current => current.map(d =>
        d.id === selectedDeckId ? { ...d, cardCount: (d.cardCount || 0) + data.saved_count } : d
      ));
      setTimeout(() => { setContent(""); setCards([]); setStatus("idle"); }, 1800);
    } catch (err) {
    ...
    <div className="min-h-screen bg-background text-text-primary font-body">
      <header className="sticky top-0 z-30 bg-background/90 backdrop-blur-md border-b border-border">
        <div className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-3">
            <Link
              href={selectedDeck ? `/dashboard/decks/${selectedDeck.id}` : "/dashboard"}
            ...
            <div>
              <h1 className="font-heading text-xl font-bold tracking-tight">Add content</h1>
              {selectedDeck && (
                <p className="text-xs text-text-secondary">→ <span className="text-primary font-semibold">{selectedDeck.name}</span></p>
              )}
            </div>
          </div>
      ...
      <main className="mx-auto grid w-full max-w-5xl gap-6 px-4 py-6 md:grid-cols-[minmax(0,1fr)_280px] md:px-6 md:py-10 animate-page-in">
        ...
        <section className="space-y-5">
          ...
          <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <motion.button
                ...
                disabled={!canGenerate || status === "generating" || status === "saving" || decks.length === 0}
              ...
              <motion.button
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.99 }}
                onClick={() => { setContent(""); setCards([]); setStatus("idle"); setError(""); }}
                disabled={status === "generating" || status === "saving"}
          ...
          <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-4">
              <div>
                <h2 className="font-heading font-semibold text-base text-text-primary">Preview cards</h2>
                <p className="text-xs text-text-secondary">
                  {cards.length > 0 ? "Click a card to flip · Edit or delete before saving." : "Review cards before saving."}
                </p>
              </div>
            ...
            <AnimatePresence mode="wait">
              {status === "generating" && (
                ...
                >
                  ...
                  <p className="font-heading font-semibold text-base text-text-primary">Analyzing your notes...</p>
              ...
              {(status === "idle" || status === "error") && (
                ...
                >
                  ...
                  <p className="font-heading font-semibold text-base text-text-primary">No generated cards yet</p>
                ...
                >
                  {cards.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 text-center">
                      ...
                      <p className="text-sm text-text-secondary">All cards deleted. Generate again or go back.</p>
                    </div>
                  ) : (
                  ...
                  {cards.length > 0 && (
                    <motion.button
                      ...
                      animate={{
                        backgroundColor:
                          status === "saved" ? "#1D9E75" : status === "saving" ? "#085041" : "#5C51E8",
                      }}
        ...
        <aside className="space-y-4">
          {/* Deck target */}
          <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm">
            <h2 className="font-heading font-semibold text-base text-text-primary mb-1">Deck target</h2>
            {selectedDeck && (
              <p className="text-xs text-text-secondary mb-3">
                Cards will be saved to <span className="text-primary font-semibold">{selectedDeck.name}</span>.
              </p>
            )}
            {decks.length === 0 ? (
              <p className="text-xs text-text-secondary leading-relaxed">
                No decks found. Please{" "}
                <Link href="/dashboard/decks" className="text-primary font-semibold hover:underline">
                  create a deck
                </Link>{" "}
                first.
              </p>
          ...
          {/* Generation status */}
          <div className="rounded-[14px] border border-border bg-card p-5 shadow-sm">
            <h2 className="font-heading font-semibold text-base text-text-primary">Status</h2>
            <ol className="mt-4 space-y-3 text-sm">
              {["Paste source", "Generate preview", "Review & edit", "Save cards"].map((item, index) => {
                const done =
                  index === 0 ||
                ...
                return (
                  <li key={item} className="flex items-center gap-3">
                    ...
                    <span className={done ? "font-semibold text-text-primary" : active ? "font-semibold text-primary" : "text-text-secondary"}>
                      {item}
                    </span>
                  </li>
                );
          ...
          {status === "preview" && cards.length > 0 && (
            ...
            >
              <p className="font-bold text-primary text-[10px] uppercase tracking-widest">Tips</p>
              <p>• <span className="text-text-primary font-semibold">Tap a card</span> to flip it</p>
              <p>• Click <Edit3 className="inline w-3 h-3 mx-0.5" /> to edit any card</p>
              <p>• Click <Trash2 className="inline w-3 h-3 mx-0.5 text-error" /> to remove unwanted cards</p>
            </motion.div>
          )}
        </aside>
      </main>
